					...
					DiskTable<S>* R_table = nullptr)
{
	...
	std::cout << get_timestamp() << "[P3-1] Table " << L_index + 1 << " took "
	...
	uint32_t memo_len)
{
	...
	fflush(file);
	std::cout << get_timestamp() << "Wrote plot header with " << num_bytes << " bytes" << std::endl;
	return num_bytes;
}
...
//		std::cout << "[P3-2] Lost " << R_num_read - L_num_write << " entries due to PMAX-bit overflow." << std::endl;
	...
	std::cout << get_timestamp() << "[P3-2] Table " << L_index + 1 << " took "
				<< (get_wall_time_micros() - begin) / 1e6 << " sec"
				...
				const std::string plot_dir)
{
	...
	std::cout << get_timestamp() << "Phase 3 took " << (get_wall_time_micros() - total_begin) / 1e6 << " sec"
