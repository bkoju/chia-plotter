					...
					const bitfield* R_used)
{
	...
	DiskTable<T> R_input(R_table);
	{
		...
		std::cout << get_timestamp() << "[P2] Table " << R_index << " scan took "
	...
	std::cout << get_timestamp() << "[P2] Table " << R_index << " rewrite took "
				...
				const std::string tmp_dir_2)
{
	...
	std::cout << get_timestamp() << "[P2] max_table_size = " << max_table_size << std::endl;
	...
	std::cout << get_timestamp() << "Phase 2 took " << (get_wall_time_micros() - total_begin) / 1e6 << " sec" << std::endl;
}
